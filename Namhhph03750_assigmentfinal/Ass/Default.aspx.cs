...
public partial class _Default : System.Web.UI.Page
{
    ...
    private void tintuc()
    {
        ...
        for (int i = 0; i < news.Count; i++)
        {
            if (i == 0)
            {
                ltlList.Text = string.Format(@"<div class='active item'><a href='TinTuc.aspx?ID={0}'><img src='{1}' alt='slide1'></a></div>",
                                    news[i]["id"], news[i]["images"]);
            }
            else
            {
                ltlList.Text += string.Format(@"<div class='item'><a href='TinTuc.aspx?ID={0}'><img src='{1}' alt='slide1'></a></div>",
                                    news[i]["id"], news[i]["images"]);
            }
        }
        conn.Close();
